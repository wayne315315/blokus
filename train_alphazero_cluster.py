...
# 2. Inference Server
# ==========================================
def training_inference_server(conns, model, device, shared_counter, total_games, shared_data_bases, total_threads, rank):
    import torch 
    ...
    while active_conns:
        ...
        if total_states_queued > 0:
            ...
            if curr_time - last_print_time > 0.5 and rank == 0:
                ...
                print(f"[Master Node] ⚡ GPU BATCH {actual_size:<5} | "
...
def run_training_pipeline(num_iteration=2000):
    ...
    dprint("🔥 Allocating Static Inter-Process Communication Arrays (Once)...", flush=True)
    ...
    if os.path.exists(model_path):
        dprint(f"🚀 Found existing model! Resuming training from {model_path}...", flush=True)
        checkpoint = torch.load(model_path, map_location=device, weights_only=True)
        model.load_state_dict(checkpoint['model_state_dict'])
        start_iteration = checkpoint.get('iteration', 1)
    else:
        dprint("🚀 No existing model found. Building a new PyTorch AdvancedBlokusModel from scratch...", flush=True)

    # 🚀 DDP CLUSTER: Wrap the training model for QSFP56 gradient sync
    model = DDP(model, device_ids=[local_rank])
    optimizer = optim.Adam(model.parameters(), lr=2e-4)
    if os.path.exists(model_path) and 'optimizer_state_dict' in checkpoint:
        optimizer.load_state_dict(checkpoint['optimizer_state_dict'])

    is_mac = sys.platform == "darwin"
    fast_infer_model = PyTorchAdvancedBlokusModel(num_blocks=BLOCKS, filters=FILTERS).to(device)

    if not is_mac and hasattr(torch, "compile"):
        import torch._dynamo
        torch._dynamo.config.cache_size_limit = 64
        
        dprint(f"🔥 Wrapping inference model with torch.compile (Triton Fusion + CUDA Graphs Enabled)...", flush=True)
    ...
    if os.path.exists(buffer_path):
        dprint(f"💾 Found existing Replay Buffer! Loading historical states from {buffer_path}...", flush=True)
    ...
    for iteration in range(start_iteration, start_iteration + num_iteration):
        ...
        # 🚀 DDP CLUSTER: Sync weights from the DDP master (.module) to the un-wrapped inference model
        fast_infer_model.load_state_dict(model.module.state_dict(), strict=False)
        fast_infer_model.eval()
        ...
        for epoch in range(epochs):
            ...
            for batch_x, batch_yv, batch_ys in dataloader:
                ...
                if device.type == 'cuda':
                    with torch.autocast(device_type='cuda', dtype=torch.float16):
                        # 🚀 DDP triggers automatic gradient averaging across the cluster during the backward pass here
            ...
            dprint(f"   Epoch {epoch+1}/{epochs} | Avg Node Loss: {epoch_loss/num_batches:.4f}", flush=True)
        
        # 🚀 DDP CLUSTER: Only Rank 0 saves the universal model checkpoint to prevent race conditions
        if rank == 0:
            dprint(f"💾 Master Node saving generation {iteration} model...", flush=True)
            checkpoint_data = {
            ...
            for old_file in glob.glob("blokus_expert_v*.pt"):
                if old_file != current_model_name and old_file != "blokus_expert_latest.pt":
                    try: 
                        os.remove(old_file)
                        dprint(f"🗑️ Auto-deleted old model: {old_file}", flush=True)
                    except OSError: pass

        # Every node saves its own local replay buffer independently
        with open(buffer_path, "wb") as f:
            pickle.dump((replay_states, replay_values, replay_scores, iteration + 1), f, protocol=pickle.HIGHEST_PROTOCOL)

        # Barrier to ensure all nodes finish the epoch before proceeding
        dist.barrier()
